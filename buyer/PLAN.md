...
- **Живой звонок** (есть в прототипе, реальная реализация на M2):
  - **Запись + транскрипт в реальном времени** — ElevenLabs real-time monitoring
    (WebSocket: события transcript/agent_response).
  - **Прослушка вживую** — Twilio Conference monitor (оператор заходит в
    конференцию muted) либо Media Streams (аудио по WebSocket в приложение).
  - **Перехват оператором** — ElevenLabs transfer-to-human (conference transfer
    / SIP REFER) или Twilio barge (размьютить ногу оператора, заглушить ИИ).
  - **Если продавец растерян** — у агента отдельная реплика: «я ИИ-ассистент,
    помогаю сэкономить время на первичном сборе информации перед заказом; этой
    инфы не нашли онлайн у вас на сайте, решение о покупке примет сам покупатель».
